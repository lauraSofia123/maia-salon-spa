...
const SocialIcons = ({ network, size = 'md', className = '' }) => {
  const Comp = socialComponents[network];
  if (!Comp) return null;
  if (network === 'tiktok' || network === 'whatsapp') {
    const px = size === 'sm' ? 14 : 20;
    return <Comp size={px} className={`${className} text-current`} />;
  }
  return <Comp className={`${iconSizeClasses[size]} ${className}`} strokeWidth={1.5} aria-hidden="true" />;
};

export default SocialIcons;
export { WhatsAppIcon, TikTokIcon };